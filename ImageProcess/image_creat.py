...
class PixelWork:
    ...
    def add_pixel(self, x, y):
        self.pix_map[(x, y)] = scipy.stats.skewnorm.rvs([self.ax, self.ay], [x, y], [self.sigma, self.sigma],
                                                        [self.signals, 2]).T
    ...
    def draw(self):
        ...
        historgram_image, xedges, yedges = np.histogram2d(x_sum, y_sum, bins=(xedges, yedges))
        # allow the file name from arguments
        ...
        cv2.imwrite(current_output_file_name, historgram_image)
